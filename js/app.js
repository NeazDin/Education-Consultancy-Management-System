...
const TITLES = {
  ...
  "portal/checkout": ["Secure Payment Checkout", "Review your selected fee before continuing to the payment provider"],
...
function getPageTitle(parts, query) {
  ...
  if (p0 === "portal") {
    ...
    if (p1 === "checkout") return "Secure Payment Checkout";
...
function route(forceTransition = false) {
  ...
  try {
    ...
    } else if (parts[0] === "portal" && parts[1] === "checkout") {
      setTitle("portal/checkout");
      Views.portalPayCheckout(root, query);
